...
function Todos({ setIsAuthenticated }) {
  ...
  async function markAll() {
    try {
      const response = await axios.patch(`${urls.markAll}/${userID}`);
      setTodos(
        todos.map((todo) => {
          if (!todo.status) {
  ...
  async function unmarkAll() {
    try {
      const response = await axios.patch(`${urls.unmarkAll}/${userID}`);
      setTodos(
        todos.map((todo) => {
          if (todo.status) {
  ...
  async function deleteAll() {
    try {
      const response = await axios.delete(`${urls.deleteAll}/${userID}`);
            ...
            <div className="todo" key={index}>
              <div>
                ...
                <div>
                  ...
                  <span
                    className="delete"
                    onClick={() => deleteTodo(todo._id, todo.status)}
                  >
                    <MdDelete />
                  </span>
                </div>
